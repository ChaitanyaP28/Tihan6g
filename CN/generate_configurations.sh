...
#echo
#echo "Registering UE 4..."
#IMSI="001010123456782"
#IMSI="${PLMN}${IMSI:$PLMN_LENGTH}" # Ensure that the beginning of the IMSI is the correct PLMN

#echo "Registering UE 4..."
#CONF_FILE="../OpenAirInterface_Testbed/User_Equipment/configs/ue4.conf"
#IMSI=$(grep -oP 'imsi\s*=\s*"\K[0-9]+' "$CONF_FILE")
#KEY=$(grep -oP 'key\s*=\s*"\K[0-9A-Fa-f]+' "$CONF_FILE")
#OPC=$(grep -oP 'opc\s*=\s*"\K[0-9A-Fa-f]+' "$CONF_FILE")
#APN=$(grep -oP 'dnn\s*=\s*"\K[^"]+' "$CONF_FILE")
#IMSI="${PLMN}${IMSI:$PLMN_LENGTH}"
#./install_scripts/register_subscriber.sh --imsi "$IMSI" --key "$KEY" --opc "$OPC" --apn "$APN"

# Optional: Adjust IMSI based on PLMN
#PLMN="00101"
#PLMN_LENGTH=${#PLMN}
#echo "Current working directory: $(pwd)"

source ../OpenAirInterface_Testbed/MyMod.conf
echo "NUM_UES is $NUM_UES"

for (( i=1; i<=NUM_UES; i++ )); do
#for i in {1..4}; do
  echo "Registering UE $i..."
  CONF_FILE="../OpenAirInterface_Testbed/User_Equipment/configs/ue${i}.conf"
...
#./install_scripts/register_subscriber.sh --imsi $IMSI --key 00112233445566778899AABBCCDDEF02 --opc 63BFA50EE6523365FF14C1F45F88737D --apn srsapn
